//this modal is deprecated and has been optimized 
//the rendering although fast makes it vulnerable to crashing if one decided to 
// quickly activate an action on a rendering card
// when clicking on a card certain options are available for a split second 
// which may or may not be available options once fully rendered and can result in errors
// with the new re worked block and new unblock functions the message area on the card 
// is exposed still and to use a logical statement to disable it becomes error prone
// as the message area must be disable for 3 conditions if the current UPC is the current user
/** themselves, a user the the current user has blocked or a user that has blocked the current
 *  user, with/ without this condtion a blank generic template of the UPC appears for a spilt
 * second and reveals the message container which is not allowed.
 * instead the whole card  is rendered through the statment instead of just the options selcection
 * section as just rendering options causes these problems, combining options and message area
 * into a fragment works well but renders a generic template for the upc but with no option selections
 * before rendering fully the correct layout after a split second. this although great is a viable 
 * method but having the whole card rendered itself allows instant rendering upon the card being clicked
 * no generic template if performance issues arise from this then the fragement method will be
 *  used instead;
 * 
 */
