...
const DmChatRoom = (props) => {
    ...
    console.log(dmParams)


    // useEffect(() => {
    //     // props.fetchDmServer(props.dmServer.id).then((dmserver) => {
    //     //     console.log("hi")
    //     //     console.log(dmserver);
    //     //     setDmMessages(Object.values(dmserver.dm_messages));
    //     //     // setDmMessagesIds(Object.values(dmserver.dm_messages).map((dm)=>dm.id));
    //     // });
    ...
    useEffect(() => {
        ...
        if (parseInt(dmParams.dmServerId) !== props.dmServer.id) {
            props.fetchDmServer(props.dmServerId);
            setDmMessages(props.DmMessages);
            setNewDmMessage(props.dmMessage);
            setDmMessagesIds(props.dmMessagesIds);
        ...
        fetchIsDmMember({ dm_member_id: props.currentUser.id, dm_server_id: props.dmServer.id }).then((result) => {
            if (result === false) {
                console.log("not member booting");
                props.removeDmServer(props.dmServerId).then(() => {
                    props.history.push('/$/$TR!F3-INTRUSION-PREVENTION/');
                });
            }
            else {
                console.log("still member");
                // unsubscribe();
                props.fetchDmServer(props.dmServerId);
                // subscribe();
                setDmMessages(props.DmMessages);
        ...
        const paramsToSend = { channel: 'DmChannel', id: props.dmServerId };
        const handlers = {
                ...
                if (dm_message !== undefined) {
                    if (Object.values(dm_message).length > 1) {
                        if (dmMessagesIds.includes(dm_message.id.toString())) {
                            ...
                            setDmMessages(prevState => DMMessagesCollection);
                            ...
                            console.log("%c state DMMessage", "color:#669999");
                            console.log(DmMessages)
                            console.log("%c state DMMessageids", "color:#669999");
                            console.log(dmMessagesIds)
                            console.log("%cdm_Message", "color:#669999");
                            console.log(dm_message);
                            let newDmMessages = DmMessages.concat([dm_message]);
                            let newDmMessagesIds = dmMessagesIds.concat(dm_message.id.toString());
                            console.log(newDmMessages);
                            console.log(newDmMessagesIds);
                            // setDmMessages([...DmMessages, [dm_message]]);
                            // setDmMessagesIds([...dmMessagesIds, [dm_message.id.toString()]]);
                            // setDmMessages(newDmMessages);
                            // setDmMessagesIds(newDmMessagesIds);
                            setDmMessages(prevState => newDmMessages);
                            setDmMessagesIds(prevState => newDmMessagesIds);
                        }
                    }
                    else {
                        ...
                        setDmMessages(prevState => filteredDmMessages);
        ...
        subscription.current = cable.subscriptions.create(paramsToSend, handlers);
        return function cleanUp () {
            subscription.current?.unsubscribe();
        }


    }, [dmParams.dmServerId, DmMessages])
