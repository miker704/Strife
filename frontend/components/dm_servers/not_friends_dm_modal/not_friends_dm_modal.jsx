...
const NotFriendsInviteToDmModal = (props) => {
    ...
    useEffect(() => {
        ...
        let friendDataReSync = props.users.find((user) => user.id === viewedUser.id);

        if (friendDataReSync) {
            if (friendDataReSync?.friend_request_status === 3) {
                props.setNotFriendsDMModal(false);
            }
            else {
                setViewedUser(friendDataReSync);
            }
        }
    ...
    let renderButton = validStatuses.includes(viewedUser.friend_request_status) ? (
        <button type="button" className="add-friends-to-dm-button-not-friends" onClick={(e) => handleCreateFriendShip(e)}>
            <div className="create-dm-button-text">{`${viewedUser.friend_request_status === 0 ? `Send Friend Request` : `Accept Friend Request`}`}</div>
        </button>
    ) : viewedUser.friend_request_status === 1 ? (
                        ...
                        <div className="create-dm-modal-focus-lock">
                            <div className="create-dm-modal">
                                ...
                                <div className="create-dm-no-friends-error-state-flex-wrapper">
                                    ...
                                    <div>You need to be friends with{` `}<strong>{viewedUser.username}</strong>{` `} to start a group DM.</div>
                                    {renderButton}
                                </div>
                            </div>
                        </div>
