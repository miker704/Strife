...
const ServerBannerModal = (props) => {

    const popUpRef = useRef(null);

    const [serverBanner, setServerBanner] = useState('');
    const [serverBannerUrl, setServerBannerUrl] = useState(props.server.server_banner);
    ...
    useEffect(() => {
        setServerBannerUrl(props.server.server_banner);

        if (props.server.server_banner === undefined) {
    ...
    useEffect(() => {
        ...
        if (props.server.server_banner !== undefined) {
            if (serverBannerUrl !== props.server.server_banner) {
                setRenderButton(saveButton);
            }
            else if (serverBannerUrl === props.server.server_banner) {
                setRenderButton(removeButton);
            }
        }
        else {
    ...
    const handleRemoveBannerSubmission = (e) => {
        ...
        if (props.server.server_banner) {
            setRemoveServerBanner(true)
            setServerBannerUrl("");
        ...
        formData.append('server[remove_SB]', removeServerBanner);
        props.changeServerBanner(props.server.id, formData).then((action) => {
    ...
    const handleFileInput = (e) => {
        ...
        fileReader.onloadend = () => {
            setServerBannerUrl(fileReader.result)
    ...
    const handleSubmit = (e) => {
        ...
        formData.append('server[server_name]', props.server.server_name);
        if (serverBanner) {
            formData.append('server[server_banner]', serverBanner);
        }
                                    ...
                                    <div className='usub-edit-banner-display-container' onClick={(e) => file_Input_Ref.current.click()}>
                                        {
                                            ...
                                            serverBannerUrl ? (
                                                <img className="usub-edit-banner-display-img" src={serverBannerUrl} alt=" " />
                                            ) : (
                                        ...
                                        <div className="usub-edit-banner-hover-hint">Upload New Server Banner</div>
...
export default ServerBannerModal;
