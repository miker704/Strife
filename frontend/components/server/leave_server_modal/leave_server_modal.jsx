...
const LeaveServerModal = (props) => {
    ...
    const handleESC = (e) => {
        const keys = {
            27: () => {
                e.preventDefault();
                document.getElementById("leave-server-modal").classList.add("transition-out");
                setTimeout(() => {
                    props.closeModal();
                    window.removeEventListener('keyup', handleESC, false);
                }, 300);
                ...
                <div id="leave-server-focus-lock" className="leave-server-focus-lock" onClick={(e) => e.stopPropagation()} ref={popupRef}>
                    <div className="leave-server-root" id="leave-server-modal">
                        ...
                        <form onSubmit={handleLeaveServer} className="leave-server-button-flex-wrapper">
                            <button type="submit" className="leave-server-submit-button">Leave Server</button>
                            <button type="button" onClick={(e) => handleCloseOnOuterClick(e)} className="leave-server-cancel-button">Cancel</button>
                        </form>
                    </div>
                </div>
            </div>
